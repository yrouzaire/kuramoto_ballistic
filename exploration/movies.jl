...
## --------------- Small Wrapper --------------- ##
function movies(param, times; particles=false, verbose=true, nb_neighbours = false)   
    system = System(param)
    anim = @animate for tt in each(times)
        ...
        plot_thetas(system,particles=true,size=(512,512), nb_neighbours = nb_neighbours)
...
evolve!(system, 10)
# evolve!(system, 20)
plot_thetas(system,particles=true,nb_neighbours=true,defects=false,vertical=false,size=(512,512))
...
z = @elapsed for init_pos in inits_pos, init_theta in inits_theta 
    ...
    anim = movies(param, times, particles=true,nb_neighbours=true,verbose=true)
    push!(anims, anim)
    filepath = "impact_init/films/$(init_theta)/B_$(init_pos)_N$(N)_ρ$(rho)_R0$(round(R0,digits=2))_T$(T).mp4"
