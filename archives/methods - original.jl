...
function initialisation(N, Lx, Ly, σ, params; float_type=Float32)
    ...
    elseif params[1] in ["sobol", "Sobol"]
        ...
        println("Error : params[2] should be \"random\" or \"square_lattice\" or \"Sobol\"")
    end
    ...
    elseif params[1] == "1Dwave"
        ...
        if params[2] == "horizontal"
            for n in 1:N
                thetas[n] = mod(params[3] * 2pi * pos[1, n] / Lx, 2pi)
            end
        elseif params[2] == "vertical"
            for n in 1:N
                thetas[n] = mod(params[3] * 2pi * pos[2, n] / Ly, 2pi)
            end
        end
    elseif params[1] == "2Dwave"
        ...
        thetas = zeros(N)
        for n in 1:N
            thetas[n] = mod(π .+ π / 2 * (sin(params[2] * pos[1, n] * 2pi / Lx) + sin(params[3] * pos[2, n] * 2pi / Lx)), 2pi)
        end
    end
...
function get_list_neighbours(pos::Matrix{T}, N::Int, Lx::Int, Ly::Int) where {T<:AbstractFloat}
    ...
    for n in 1:N
        ...
        # sizehint!(ind_neighbours_n, 10) # does not improve much
        for (i, j) in get_neighbouring_cells(cellx, celly, nb_cells_x, nb_cells_y)
            ...
            next = head[i, j]
            if next ≠ -1
                if 0 < dist(poscur, pos[:, next], Lx, Ly) ≤ R0
                    push!(ind_neighbours_n, next)
                end
                while list[next] ≠ -1
                    if 0 < dist(poscur, pos[:, list[next]], Lx, Ly) ≤ R0
...
function update_DefectTracker!(dt::DefectTracker, pos::Matrix{T}, thetas::Vector{T}, N, Lx, Ly, t) where {T<:AbstractFloat}
    ...
    elseif N_new > 0 && N_old == 0
        for i in 1:Np_new
            add_defect!(dt, charge=chargeP_new[i], loc=locP_new[i])
        end
        for i in 1:Nn_new
            add_defect!(dt, charge=chargeN_new[i], loc=locN_new[i])
        end
    ...
    elseif Nn_new > 0 && Nn_old > 0 && Np_old > 0 && Np_new == 0  # (+)(-)(-) >> (-) par exemple
        ...
        for i in eachindex(assignment_antivortices)
            update_position!(dt.defectsN[assignment_antivortices[i]])
        end
        # Identify annihilated defects
        ID_annihilated_vortices = []
        ID_annihilated_antivortices = []
        ...
        elseif N_new > N_old
            ...
            for j in each(loc_created_vortex)
                add_defect!(dt, charge=chargeP_new[j], loc=loc_created_vortex[j][1:2])
            end
            ...
            for j in each(loc_created_antivortex)
                add_defect!(dt, charge=chargeN_new[j], loc=loc_created_antivortex[j][1:2])
            end
